...
- `npm run expo:tunnel` / `npm run expo:tunnel:go` — tunnel mode (ngrok-backed) when LAN isn't reachable (client isolation, different subnets, phone on mobile data). Use the `:go` variant for Expo Go: plain `expo start --tunnel` omits `--go`, so with `expo-dev-client` installed it advertises a dev-client URL that Expo Go refuses to open — the same trap `expo:go` avoids on the LAN path. **If the tunnel cannot be established, Expo prints `CommandError: ngrok tunnel took too long to connect` and exits — it does not fall back to LAN, so nothing is left listening.** The phone then reports `ERR_NGROK_3200 — the endpoint is offline`, which looks like a bad QR but means no server is running; on a network that blocks ngrok, use LAN mode instead
