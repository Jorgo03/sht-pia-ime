...
- `npm run expo:qr` — writes the Expo Go QR to an SVG page and opens it, because Metro's terminal QR is drawn with block characters and frequently will not scan from Windows terminals. Refuses to emit a code unless a dev server actually answers on the port. `-- --host=<addr>` to override (a tunnel host takes no port; a LAN IP does)
