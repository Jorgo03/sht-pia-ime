...
- `npm run expo:tunnel` / `npm run expo:tunnel:go` — tunnel mode (ngrok-backed) when LAN isn't reachable (client isolation, different subnets, phone on mobile data). Both route through `start-expo-lan.cjs --tunnel`, the single spawn site, so the Expo Go redirect-page rule below applies to them too. Use the `:go` variant for Expo Go: plain `expo start --tunnel` omits `--go`, so with `expo-dev-client` installed it advertises a dev-client URL that Expo Go refuses to open — the same trap `expo:go` avoids on the LAN path. **A tunnel fails in two different ways, and only one is loud.** Loud: `CommandError: ngrok tunnel took too long to connect`, and Expo exits, leaving nothing listening — the phone then reports `ERR_NGROK_3200`. Quiet, and worse: `Tunnel URL not found (it might not be ready yet), falling back to LAN URL` — Expo keeps running and advertises a **LAN** address, so on the very network you reached for a tunnel to escape, you are handed the address that network blocks. **This is why `expo:tunnel:qr` is the command to use on a restrictive network**: it verifies the public `.exp.direct` host is really serving before it draws anything, so the quiet fallback cannot reach your phone as a QR
- **When neither LAN nor a tunnel works, the phone's hotspot does.** Join the laptop to the phone's hotspot and both are on one tiny network nobody else administers: no client isolation, no VLAN split, no inbound filter, and no ngrok in the path — so plain `npm start` works and the bundle transfers at LAN speed rather than round-tripping through a relay. It is the fallback with the fewest moving parts, not a last resort, and it is the answer when a corporate network blocks ngrok as well as peer traffic. Log in to Expo (`npx expo login`) from a network that permits it before relying on the tunnel elsewhere — the credential persists, so it is once per machine, not once per network
- **Which command to use is decided by the network, not by preference**: `npm start` (LAN) at home — fastest, since the bundle goes straight over Wi-Fi; `npm run expo:tunnel:qr` on a restrictive network (corporate/guest Wi-Fi with AP client isolation, separate VLANs for wireless clients, or inbound filtering), where LAN cannot work no matter what the project does. Nothing in this repo can defeat client isolation or a firewall — that is a property of the network, and the tunnel is the supported way around it, at the cost of routing the bundle through ngrok
- **`npm start` — the one command for getting the app onto a phone.** Starts Metro in Expo Go mode, waits until it genuinely answers, then opens a scannable QR built from the address the server is actually on. Always regenerate rather than reusing a saved QR: the code encodes your machine's LAN address, and DHCP hands out a different one after a reconnect or a network change, which makes a saved code fail in a way that looks like a broken QR. The page polls the server and shows a live green/red state, so a stale code says so instead of silently failing
- **The same two, named explicitly:**
  - `npm run expo:go:qr` — same Wi-Fi. Starts Metro, waits until `/status` actually answers, then opens the QR
  - `npm run expo:tunnel:qr` — **any network** (different Wi-Fi, mobile data, client-isolated Wi-Fi). Starts a tunnelled server and, before drawing anything, verifies the public `.exp.direct` host is really serving. Needs `npx expo login`: the tunnel address embeds your account name
- `npm run expo:qr` — QR only, against a server you started yourself. Writes an SVG page rather than relying on Metro's terminal QR, which is drawn with block characters and frequently will not scan from Windows terminals. Refuses to emit a code unless a dev server answers. `--target=dev-client`, `--tunnel=<host>`, `--host=<addr>`, `--print`
- The URL in every case is built by `@expo/cli`'s own `UrlCreator`, never assembled by hand, so it honours `REACT_NATIVE_PACKAGER_HOSTNAME` (what `start-expo-lan.cjs` sets), `EXPO_PACKAGER_PROXY_URL`, Expo's `getIpAddress()`, and the rule that a tunnel host carries its port in the subdomain and must not have one appended
...
- **i18n** — zero hardcoded UI strings, everything through `t('...')` · DB content translation goes through the `translate-property` Edge Function, cached — one call per target language carrying title **and** description together, never one call per field · listing translations are cached against a source fingerprint in `properties.translation_meta`, and a translation an agent edited by hand is never regenerated without an explicit action · a key added on one platform's code path needs the underlying locale JSON key to exist for both, since the files are shared.
